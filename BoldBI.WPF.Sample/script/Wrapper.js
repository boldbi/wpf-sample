...
class BoldBI {
    ...
    */
    exportDashboardAsPdf(dashboardId, fileName, pageSize, pageOrientation, showAppliedFilters) {
        if (this.isMultiTab) {
            ...
            if (dbrdInstance != undefined) {
                dbrdInstance.exportAsPdf(fileName, pageSize, pageOrientation, showAppliedFilters);
            }
        }
        else {
    ...
    *  @param {boolean} showAppliedFilters - Define whether we need to export the dashboard with or without a filter
    */
    exportDashboardAsImage(dashboardId, fileName, exportImageFormat, resolutionDpi, showAppliedFilters) {
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportAsImage(fileName, exportImageFormat, resolutionDpi, showAppliedFilters);
            }
        }
        else {
            var dbrdInstance = this._getDashboardInstance();
    ...
    */
    exportDashboardAsExcel(dashboardId, fileName, fileType) {
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportAsExcel(fileName, fileType);
            }
            else {
                var dbrdInstance = this._getDashboardInstance();
                if (dbrdInstance != undefined) {
    ...
    *  @param {boolean} showAppliedFilters - Define whether we need to export the dashboard with or without a filter.
    */
    exportWidgetAsPdf(dashboardId, widgetName, fileName, pageSize, pageOrientation, showAppliedFilters) {
        var that = this;
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportWidgetAsPdf(widgetName, fileName, pageSize, pageOrientation, showAppliedFilters);
            }
        }
        else if (bbEmbed('.pinBoardDbrd').length > 0) {
            var length = bbEmbed('.pinBoardDbrd').length;
            for (var i = 0; i < length; i++) {
                if (that.pinboardIds[i].widgetId == dashboardId) {
    ...
    * @param {string} dashboardId - Define the unique id of the dashboard if it is present within the multitab dashboard or widget id present in the pinboard
     ...
     *  @param {boolean} showAppliedFilters - Define whether we need to export the dashboard with or without a filter.
    */
    exportWidgetAsImage(dashboardId, widgetName, fileName, exportImageFormat, resolutionDpi, showAppliedFilters) {
        var that = this;
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportWidgetAsImage(widgetName, fileName, exportImageFormat, resolutionDpi, showAppliedFilters);
            }
        }
        else if (bbEmbed('.pinBoardDbrd').length > 0) {
            var length = bbEmbed('.pinBoardDbrd').length;
    ...
    *  @param {string} fileType - Define the type of file to be exported ('xlsx','xls').
    */
    exportWidgetAsExcel(dashboardId, widgetName, fileName, fileType) {
        var that = this;
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportWidgetAsExcel(widgetName, fileName, fileType);
            }
        }
        else if (bbEmbed('.pinBoardDbrd').length > 0) {
            var length = bbEmbed('.pinBoardDbrd').length;
    ...
    * @param {string} fileName - Define the name of the file to be exported
    */
    exportWidgetAsCsv(dashboardId, widgetName, fileName) {
        var that = this;
        if (this.isMultiTab) {
            var MultitabDashboardId = 'multi_' + dashboardId + '_embeddedbi';
            var dbrdInstance = this._getDashboardInstance(MultitabDashboardId);
            if (dbrdInstance != undefined) {
                dbrdInstance.exportWidgetAsCsv(widgetName, fileName);
            }
        }
        else if (bbEmbed('.pinBoardDbrd').length > 0) {
            var length = bbEmbed('.pinBoardDbrd').length;
    ...
    refreshWidget(widgetnames, hideLoader, dashboardId) {
        var that = this;
        ...
        if (!responseInfo.Status) {
            ...
            if (this.embedOptions.pinboardName !== "" && this.pinboardIds.length === 0) {
                ...
                if (this.pinboardIds.length > 0) {
                    ...
                    if (typeof this.embedOptions.dashboardSettings.dashboardName != 'string') {
                    ...
                    if (typeof this.embedOptions.dashboardSettings.dashboardName === 'string') {
                        dashboardName = this.embedOptions.dashboardSettings.dashboardName;
                    }
                }
    ...
    _renderPinboard(itemDetail) {
        ...
        bbEmbed.map(that.pinboardvalue, function (value) {
    ...
    changeLayout(layout) {
        ...
        if (!that._isEmptyOrSpaces(that.embedOptions.authorizationServer.url)) {
            ...
            bbEmbed.ajax({
                async: false,
                type: 'POST',
                url: 'http://localhost:51778/bi/api/site/site1/embed/pinboard',
                headers: {
                ...
                if (column[i].Item.length > 0) {
                    bbEmbed.each(column[i].Item, function (j) {
                        if (column[i].Item[j].ItemExtension.toLowerCase() !== ".sydj") {
                            ...
                            that.pinboardvalue[index++] = column[i].Item[j].Widgetdetails[column[i].Item[j].ItemId];
    ...
    _renderMultiTabDashboard(embedResponse) {
        if (this.embedOptions.mode === BoldBI.Mode.View && !this.isWidgetMode) {
            ...
            bbEmbed.map(embedResponse, function (value, index) {
                ...
                if (typeof that.embedOptions.dashboardSettings.dashboardName != 'string') {
                    bbEmbed.map(that.embedOptions.dashboardSettings.dashboardName, function (val, index) {
        ...
        for (var i = 0; i < bbEmbed("#" + containerName + " .e-toolbar-item").length; i++) {
            ...
            if (bbEmbed("#" + containerName + " .e-toolbar-item.e-active").attr("aria-controls") === "e-content-" + containerName + "_" + i) {
                ...
                if (dbrdInstance === null || dbrdInstance === undefined) {
                    ...
                    var response = new ApiResponse();
                    response.Apistatus = true;
                    response.Data = this.dashboardDetails[dashboardId];
                    response.Status = true;
